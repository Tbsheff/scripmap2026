...
function BookCard({ volume, book }: {
    ...
    return (
        ...
        >
            <span className="book-abbr block font-sans text-[0.65rem] font-bold tracking-[0.12em] uppercase text-[var(--on-surface-variant)] mb-1 max-sm:hidden">{book.citeAbbr}</span>
    ...
    return (
        <section className="vol-section">
            <div className="flex items-end justify-between mb-6 pb-4 border-b border-[var(--outline-variant)] border-l-[3px] border-l-[var(--primary)] pl-4">
                <div>
                    <span className="block font-sans text-[0.7rem] font-bold tracking-[0.15em] uppercase text-[var(--primary)] mb-1">{label}</span>
                    <h2 className="font-serif text-[1.875rem] max-sm:text-2xl font-bold text-[var(--on-surface)] m-0 tracking-[-0.01em]">{volume.fullName}</h2>
                </div>
            </div>
